...
export type HealthResponse = {
  ...
  services: Record<"ollama" | "zhipu" | "redis" | "postgresql", {
...
async function request<T>(path: string, auth = false): Promise<T> {
  ...
  if (!response.ok) {
    const detail = (body as { detail?: { error?: string; message?: string } })
      ?.detail
...
export const api = {
  ...
  healthz: () =>
    request<{ status: string; timestamp?: string; uptime_seconds?: number }>(
      "/healthz",
    ),
  ...
  modelsSummary: () =>
    request<Record<string, unknown>>("/v1/models/summary", true),
  ...
  routerHealth: () =>
    request<Record<string, unknown>>("/v1/router/health", true),
